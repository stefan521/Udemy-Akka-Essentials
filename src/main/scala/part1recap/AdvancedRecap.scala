...
object AdvancedRecap extends App {
  ...
  // implicits are nice but they might confuse us if we don't organize them properly
  // implicit from local scope
  implicit val inverseOrdering: Ordering[Int] = Ordering.fromLessThan(_ > _)
  List(1, 2, 3).sorted // List(3, 2, 1)

  // implicits from imported scope
  import scala.concurrent.ExecutionContext.Implicits.global
  val future = Future {
    println("hello, future")
  }

  // implicits from companion objects of the types included in the call
  object Person {
