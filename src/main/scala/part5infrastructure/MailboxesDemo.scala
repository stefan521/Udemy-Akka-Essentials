...
object MailboxesDemo extends App {
  ...
  // step 2 - make it known in configuration in a dispatcher
...
//  supportTicketLogger ! "[P1] do this when you have the time"
  ...
  val altControlAwareActor = system.actorOf(Props[SimpleActor], "alternativeControlAwareActor") // watch Mailboxes video.
