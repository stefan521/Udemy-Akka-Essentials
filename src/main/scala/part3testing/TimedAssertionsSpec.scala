  ...
  "A worker actor" should {
    ...
    "reply to a test probe in a timely manner" in {
      within(1 second) {
        ...
        //probe.expectMsg(WorkResult(42)) // timeout of 0.3 seconds set in application.conf so it will failt
      }
    }
  }
}
...
object TimedAssertionsSpec {
  ...
  class WorkerActor extends Actor {
    override def receive: Receive = {
          // simulates an actor doing something expensive
      case "work" =>
        // long computation
        Thread.sleep(500) // is stupid in practice
        sender ! WorkResult(42)

        // simulates an actor replying in small chunks rapidly
      case "workSequence" =>
        val r = new Random()
        for (_ <- 1 to 10) {
